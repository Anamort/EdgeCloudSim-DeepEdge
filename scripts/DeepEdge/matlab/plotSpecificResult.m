function [] = plotSpecificResult(rowOfset, columnOfset, yLabel, appType, calculatePercentage)
    ...
    for s=1:numOfSimulations
        for i=1:size(scenarioType,2)
            for j=1:numOfMobileDevices
                    ...
                    readData = dlmread(filePath,';',rowOfset,0);
                    value = readData(1,5); % for service time we set columnSet as 5 

                    all_results(s,i,j) = value;
            end
        end
    end
    serviceTimeresults = mean(all_results); %still 3d matrix but 1xMxN format
    serviceTimeresults = squeeze(serviceTimeresults); %remove singleton dimensions
    %bar(types, results(1,:), 0.4);
    %bar(results(1,:), types, 0.4);
    
    markers = getConfiguration(50);
            for j=1:size(scenarioType,2)
                plot(results(1,:), serviceTimeresults(j,:),char(markers(j)),'MarkerFaceColor',getConfiguration(20+j),'color',getConfiguration(20+j));
                hold on;
            end
    ...
    for j=1:size(scenarioType,2)
            if(getConfiguration(12) == 1)
                ...
                plot(results(1,:), serviceTimeresults(j,:),':k','color',getConfiguration(20+j),'LineWidth',1.5);
            end
            hold on;
        end
    ...
    ylabel(yLabel);
    %set(gca,'XLim',[startOfMobileDeviceLoop-5 endOfMobileDeviceLoop+105]);
    set(gca,'XLim',[0 81000]);
