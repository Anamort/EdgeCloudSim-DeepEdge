...
public class NetworkingModel extends NetworkModel {
    ...
    public NetworkingModel(int numberOfMobileDevices, String simScenario){
    ...
    public double getDownloadDelayForTraining (int sourceDeviceId, int destDeviceId, Task task){
        double delay = 0;

        //special case for man communication
        if(sourceDeviceId == destDeviceId && sourceDeviceId == SimSettings.GENERIC_EDGE_DEVICE_ID){
            return delay = getManDownloadDelayForTraining();
        }
    ...
    public double getWanBandwidthForNextState(int sourceDeviceId, int destDeviceId, Task task) {
        ...
        if(numOfWanUser + 1 < experimentalWanDelay.length)
            result = taskSizeInKb /*Kb*/ / (experimentalWanDelay[numOfWanUser+1]) /*Kbps*/;
        //resultDebug = taskSizeInKb /*Kb*/ / (experimentalWanDelay[numOfWanUser]) /*Kbps*/;
    ...
    private double getManDownloadDelayForTraining() {
        double result = calculateMM1(SimSettings.getInstance().getInternalLanDelay(),
                MAN_BW,
                ManPoissonMeanForDownload,
                avgManTaskOutputSize,
